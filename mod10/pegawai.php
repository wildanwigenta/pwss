...
class Pegawai {
 ...
 $query = "SELECT id_pegawai, nama, alamat FROM " . $this->table_name
...
alamat=:alamat";
 ...
 $query = "SELECT nama, alamat FROM " . $this->table_name . " WHERE
id_pegawai = ? LIMIT 0,1";
 $stmt = $this->conn->prepare( $query );
 ...
 $query = "UPDATE " . $this->table_name . " SET nama = :nama, alamat
= :alamat WHERE id_pegawai = :id_pegawai";
 $stmt = $this->conn->prepare($query);
 // Sanitize data
 $this->nama = htmlspecialchars(strip_tags($this->nama));
 $this->alamat = htmlspecialchars(strip_tags($this->alamat));
 $this->id_pegawai = htmlspecialchars(strip_tags($this-
>id_pegawai));
 // Bind values
 $stmt->bindParam(':nama', $this->nama);
 $stmt->bindParam(':alamat', $this->alamat);
